...
def draw_screen(stdscr,tabs,selected_tab,window_parent_dir,window_current_dir,window_preview):
    ...
    for idx,tab in enumerate(tabs):
        if tab != None:
            if idx == selected_tab:
                stdscr.addstr(0,curses.COLS-len([1 for tab in tabs if tab != None])+i,str(idx+1),curses.A_REVERSE)
            else:
                stdscr.addstr(0,curses.COLS-len([1 for tab in tabs if tab != None])+i,str(idx+1))
            i += 1
